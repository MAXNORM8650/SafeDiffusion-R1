...
# Make sibling scripts importable when run as `python evaluation/execs/generate_images.py`.
...
if __name__=='__main__':
    ...
    parser.add_argument('--ckpt', help='UNet checkpoint to swap in. Accepts either '
                        '(a) a `.safetensors`/`.pt` UNet state_dict, or '
    ...
    # Resolve `--base`: shorthand → HF model id, otherwise pass through
    # (either a HF id or a local path is fine — diffusers handles both).
    if base == '1.4':
    ...
    if ckpt is not None:
        if os.path.isdir(ckpt):
            ...
            # Standalone UNet weights file — needs the base UNet config from `model`
            from safetensors.torch import load_file as _load_safetensors
            sd = _load_safetensors(ckpt)
            model.unet.load_state_dict(sd, strict=False)
        elif ckpt.endswith(('.pt', '.bin', '.ckpt')):
            sd = torch.load(ckpt, map_location='cpu')
            if isinstance(sd, dict) and 'state_dict' in sd:
                sd = sd['state_dict']
            model.unet.load_state_dict(sd, strict=False)
        else:
            raise ValueError(f'Unrecognised --ckpt format: {ckpt}')
