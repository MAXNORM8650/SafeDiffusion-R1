...
# `torchrun fastvideo/train.py`, so `from rewards.inference_reward import ...`
# works without requiring `pip install -e .`.
...
## Only for LLaVa penalty
# import sglang as sgl
# from sglang import RuntimeEndpoint
# import json
# from judgeprompts.system import *
    ...
    if config.reward_fn == 'hpsv2':
        ...
        def initialize_model():
            ...
            model, preprocess_train, preprocess_val = create_model_and_transforms(
                'ViT-L-14',
                './hps_ckpt/safeclip/open_clip_pytorch_model.bin',
                precision='amp',
                device=device,
                jit=False,
        ...
        # cp = huggingface_hub.hf_hub_download("CompVis/stable-diffusion-safety-checker")
        # cp = "./hps_ckpt/safeclip/open_clip_pytorch_model.bin"

        # checkpoint = torch.load(cp, map_location=f'cuda')
        # model.load_state_dict(checkpoint['state_dict'])
        processor = get_tokenizer('ViT-L-14')
        reward_model = model.to(device)
        reward_model.eval()
    elif config.reward_fn == "nsfwv2":
        from rewards.inference_reward import get_reward
        from rewards.inference_reward import initialize as init_nsfw
        init_nsfw()
        print("****************************************************")
        print("Using NSFW reward")
        print("****************************************************")
    ...
    for epoch, prompts in enumerate(loader):
        ...
        for i in range(0, len(expanded_prompts), batch_size):
            ...
            if i%config.num_generations == 0:
                ...
                # FIXED: Generate fresh noise for each sample to encourage diversity
            ...
            for j, image in enumerate(images):
                ...
                root_dir = config.sample_image_dir
                os.makedirs(root_dir, exist_ok=True)
                rank_id = dist.get_rank() if dist.is_initialized() else 0
                image_path = os.path.join(root_dir, f"image-{i}-{j}-rank-{rank_id}.jpg")
                pil.save(image_path)
                #Lets know if the image is safe or not first for llava penality
                ####### start of safety check ########
                # backend = RuntimeEndpoint(f"http://localhost:8100")
                # sgl.set_default_backend(backend)
                # out = guard_gen.run(image_path=image_path, prompt=prompt_gpt)
                # data = json.loads(out["json_output"])
                # rating = data["rating"]
                ######## end of safety check #########
                    ...
                    with torch.amp.autocast('cuda'):
                        if config.reward_fn == "hpsv2":
                            ...
                            reward = get_reward(
                                ...
                                current_batch[j],
                                return_tensor=True,
                                steering_alpha=config.train.steering_alpha,
                            )

                            r = float(reward.detach().cpu().item())
                            root, ext = os.path.splitext(image_path)
                            new_path = f"{root}_{r:.3f}{ext}"   # e.g. img_0.123.png
                            pil.save(new_path)
        ...
        if rank_id == 0:
            ...
            with open(config.reward_log_file, 'a') as f:
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(unet):
                        ...
                        # loss+= 0.5 * torch.mean((log_prob - sample["log_probs"][:, j]) ** 2)
                        # debugging values
